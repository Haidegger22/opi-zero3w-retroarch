...
### Ещё две грабли, найденные на второй день (исправлено)

1. **Мост калибровал джойстик, пока драйвер ещё держал шину.** Средний замер уезжал
   на сотни отсчётов (взял X=31980 вместо реальных 32368) — и персонаж в игре бежал
   сам по себе. Теперь сторож ждёт **реальной** остановки `m5hub` (опрос
   `systemctl is-active` в цикле) и даёт шине полсекунды покоя, а калибровка берёт
   **медиану**, а не среднее: одиночный сбойный замер больше не сдвигает центр.
2. **Первый ответ после переключения канала PaHub приходит от предыдущего канала.**
   Мост принимал его за данные джойстика или за маску клавиатуры — и в игру уходила
   ложная стрелка («персонаж бежит назад»). Теперь перед полезным замером идёт один
   холостой, сбросный.

Плюс цикл джойстика больше не действует по одиночному замеру: пока не накоплено три
согласованных, стрелка не жмётся. Проверка после правки: мост взял центр
X=32365 Y=33360 при реальных 32368/33364 — попадание в три отсчёта из 65536.
