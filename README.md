...
Клавиатура CardKB — «одноразовая»: она шлёт код клавиши **один раз** при нажатии и сразу сбрасывается. m5hub физически не может передать «клавиша удерживается» → в играх всё срабатывало на отпускание. Поэтому прыжок и бег вынесены на **честные GPIO-кнопки** (уровень «нажата/не нажата» читается напрямую).
